...
n=1
numeros_futuristicos = [] # lista de números para calcular su factorial

while(n != 0):
    n = int(input("\nIngrese un número entero para calcular su factorial (0 si desea salir): "))
    if n > 0 and n <= 366:  # validacion de rango
        numeros_futuristicos.append(n)
    elif n != 0:
        print("Número inválido. Intente de nuevo.")

for n in numeros_futuristicos:
    factorial = 1
    for i in range(1, n + 1):
        factorial *= i  # calculo del factorial
    print(f"{n}! -- ")
    for i in range(10):  # recorrido de los digitos
        print(f"   ({i}) {str(factorial).count(str(i)):>4} ", end="")  # conteo de ocurrencias de cada dígito en el factorial
        if i == 4 or i == 9:  # salto de línea después del cuarto y noveno dígito
            print()
